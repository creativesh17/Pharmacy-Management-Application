...
@component('admin.includes.breadcrumb')
    <h4 class="pull-left page-title">Refund Reports</h4>
    <ol class="breadcrumb pull-right">
        <li><a href="{{ url('admin/reports/refunds') }}">Refund Reports</a></li>
    ...
    <div class="col-12">
        <div class="card">
                ...
                <div class="row">
                    <div class="col-md-8">
                        @if(auth()->user()->role_id <= 2)
                            <h3 class="card-title card-top-title"><i class="fa fa-gg-circle"></i> Refund Report For {{ request('branch') }}</h3>
                        @else
                            <h3 class="card-title card-top-title"><i class="fa fa-gg-circle"></i> Refund Report For {{ $branch->branch_code }}</h3>
                        @endif
                    </div>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
                        <form action="{{ route('admin.reports.refundsearch') }}" method="get">
                            {{-- @csrf --}}
                            <div class="row">
                                @if (auth()->user()->role_id <= 2)
                                <div class="col-sm-2">
                            ...
                            <table class="table table-bordered table-striped custom-table mb-0" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                ...
                                @foreach ($refunds as $key=>$data)
                                    <tr>
                                        ...
                                        <td class="text-right">{{ $data->refund_nettotal }}</td>   
                                    </tr>
                                @endforeach
                                </tbody>
                                <tfoot>
                                    <tr>
                                        ...
                                        <td class="text-right" style="font-weight: 700; font-size: 20px;"> {{ $refundTotal }} </td>
                                    </tr>
            ...
            <div class="card-footer card-footer-part" style="background-color: #fff">
                <div class="row" style="padding-bottom: 5em; padding-left: 2em;">
                    <div class="col-md-2 col-sm-2 offset-md-10 offset-sm-10">
                        @if (auth()->user()->role_id <= 2)
                            {{ $refunds->appends(['branch' => request('branch'), 'from' => request('from'), 'to' => request('to'), 'action' => 'search'])->links() }}
                        @else
                            {{ $refunds->appends(['from' => request('from'), 'to' => request('to'), 'action' => 'search'])->links() }}
                        @endif
                    </div>
                </div>
                <div class="row">
