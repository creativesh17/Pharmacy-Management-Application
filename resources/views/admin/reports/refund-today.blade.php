    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header card-header-part">
                <div class="row">
                    <div class="col-md-8">
                        <h3 class="card-title card-top-title"><i class="fa fa-gg-circle"></i> Today's Refund Report</h3>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped custom-table mb-0" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                ...
                                <tbody>
                                @foreach ($todayRefund as $key=>$data)
                                    <tr>
                                        <td>{{ $todayRefund->firstItem() + $key }}</td>   
                                        <td>{{ $data->id }}</td>   
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td class="text-right" style="font-weight: 700; font-size: 20px;"> {{ $todayRefundTotal }} </td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
            ...
            <div class="card-footer card-footer-part" style="background-color: #fff">
                <div class="row" style="padding-bottom: 5em; padding-left: 2em;">
                    <div class="col-md-2 col-sm-2 offset-md-10 offset-sm-10">
                        @if (auth()->user()->role_id <= 2)
                            {{ $todayRefund->appends(['branch' => request('branch'), 'from' => request('from'), 'to' => request('to'), 'action' => 'search'])->links() }}
                        @else 
                            {{ $todayRefund->appends(['from' => request('from'), 'to' => request('to'), 'action' => 'search'])->links() }}
                        @endif
                    </div>
                </div>
                <div class="row">
